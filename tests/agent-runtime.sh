...
make_worktree() {
  local task="$1"
  local branch="$2"
  local path="$project_home/tasks/$task/worktree"
...
cleanup_output TASK-notes | grep -q "notes/metadata" || fail "notes task was not refused"

unpushed_path="$project_home/tasks/TASK-unpushed/worktree"
